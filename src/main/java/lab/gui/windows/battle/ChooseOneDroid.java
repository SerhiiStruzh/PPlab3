...
public class ChooseOneDroid extends JFrame
{
	...
	public ChooseOneDroid(Droid droid, Player player, String message) 
	{
		this.active = droid;
		this.player = player;
	...
	private void addButtonListener(ArrayList<JButton> buttons) 
	{
		for(int i = 0; i < buttons.size(); i++) {
			...
			buttons.get(i).addActionListener(new ActionListener() {
	        	...
	        	{
					Droid chosen_droid = player.getDroid(index);
					active.simpleAbility(chosen_droid, player);
					if(!chosen_droid.isAlive()) {
						player.removeDroid(chosen_droid);
					}
					active.increaseEnergy();
					dispose();
	            }
			});
		}
	...
	{
		for(Droid players_droid : player.getDroids()) 
		{
